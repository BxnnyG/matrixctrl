...
## Handover — the repository settings

Done by the operator on 2026-08-01: **topics** and **description**.
...
The rest should not be clicks at all. Publishing a release by hand is exactly the
failure §4.17 removed from the version pipeline, and it would come back here in a
different shape:

- **GitHub Releases** — `POST /repos/{owner}/{repo}/releases`, body from
  [CHANGELOG.md](../../CHANGELOG.md). Needed for `v0.1.15` and `v0.1.16`, and for
  every tag after them.
- **Tabs** — `PATCH /repos/{owner}/{repo}` with `has_wiki=false`,
  `has_projects=false`, `has_discussions=true`. Three empty tabs read as an
  abandoned project; Discussions is the one worth keeping, because a wiki becomes
  a second documentation that rots next to `docs/`.

Both need a fine-grained token for this repository with **Contents: read+write**
(releases) and **Administration: read+write** (repository settings). It must not
be pasted into a chat transcript or committed — it belongs in a file outside the
repository, readable only by its owner, with a short expiry.

*The release body should eventually be generated by the release workflow from the
CHANGELOG rather than posted by anyone, human or agent — same argument as §4.17.
Tracked as P2-18.*
